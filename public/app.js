...
function renderLogin() {
  appEl.innerHTML = `
    <section class="card card--gradient" aria-labelledby="login-title">
      <h2 id="login-title">Sign in to Pi Duel Arena</h2>
      <p class="subtitle">Pick a mock username to receive a 1,000 π practice balance and unlock the lobby.</p>
      <form id="login-form" class="login-form">
        <label for="username">Username</label>
        <input type="text" id="username" name="username" minlength="3" maxlength="24" required placeholder="e.g. syrup-warrior" />
  ...
  appEl.innerHTML = `
    <section class="card card--hero">
      <div class="hero">
        <div class="hero__content">
          <p class="hero__eyebrow">Arena dashboard</p>
          <h2>Heroic Pi duels await</h2>
          <p class="subtitle">Spin up an escrow lobby, challenge friends, or drill with Arena AI before Pi Network mainnet integration.</p>
          <div class="hero__actions">
            <button class="btn btn-primary" data-view="lobbies">Open lobby hub</button>
            <button class="btn btn-ghost" data-view="tournaments">Browse tournaments</button>
          </div>
          <ul class="hero__chips">
            <li>Mock π escrow · 10% house fee</li>
            <li>Best-of-3 · 10s move timer</li>
            <li>Wallet support coming soon</li>
          </ul>
        </div>
    ...
    <section class="card">
      <div class="section-title">
        <h3>Quick stats</h3>
        <span>Track your duel momentum</span>
      </div>
    ...
    <section class="card card--gallery">
      <div class="section-title">
        <h3>Why players choose Pi Duel Arena</h3>
        <span>PancakeSwap-inspired experience for the Pi community</span>
      </div>
      <div class="gallery-grid">
        ${
          [
            {
              title: 'Escrow and instant settlements',
              copy: 'Buy-ins lock as soon as a lobby fills. We keep 10% and the champion takes the rest automatically.',
              className: 'gallery-card--escrow',
            },
            {
              title: 'AI drills that feel real',
              copy: 'Arena AI adapts to your patterns so you can rehearse before wagering against live opponents.',
              className: 'gallery-card--ai',
            },
            {
              title: 'Tournaments with re-entry',
              copy: 'Jump back in as many times as you like, climb the leaderboard, and split the pot with the top three.',
    ...
    <section class="card card--secondary">
      <div class="section-title">
        ...
        <span>Live top five standings</span>
      </div>
                ...
                .map((tournament) => {
                  ...
                  return `
                    <article class="tournament-card">
                      ...
                      <p class="subtitle">${escapeHtml(tournament.location)} · ${tournament.participants}/${tournament.slots} players · ${tournament.totalEntries} entries</p>
                      <p class="subtitle">Current pot ${formatPi(tournament.currentPot || 0)}</p>
...
function renderLobbyView() {
  ...
  appEl.innerHTML = `
    ...
    <section class="card card--ai">
      <div class="section-title">
        <h3>Persistent RPS vs Arena AI</h3>
      ...
      <p class="subtitle">Arena AI adapts to your move history, so you can rehearse strategies before sending live invites.</p>
    </section>
    <section class="card">
      <div class="section-title">
        <h3>Live lobbies</h3>
                ...
                .map((tournament) => {
                  ...
                  const payoutNote =
                    tournament.status === 'completed' && tournament.payouts && tournament.payouts.length > 0
                      ? `<p class="subtitle">Payouts: ${tournament.payouts
                          .map((payout, index) => `#${index + 1} ${escapeHtml(payout.username)} ${formatPi(payout.reward)}`)
                          .join(', ')}</p>`
                      : '';
                  const scheduleLabel =
                    tournament.status === 'completed'
                      ? `Completed ${timeAgo(tournament.completedAt)} ago`
                  ...
                  return `
                    <article class="tournament-card">
                      ...
                      <p class="subtitle">${escapeHtml(tournament.location)} · ${statusText} · ${scheduleLabel}</p>
                      <p class="subtitle">Pot ${formatPi(tournament.currentPot || 0)} · Entries ${tournament.totalEntries} · Your runs ${
                    tournament.userEntries || 0
  ...
  appEl.innerHTML = `
    <section class="card">
      <div class="section-title">
        ...
        <span>Share match highlights and find new rivals</span>
      </div>
      <div class="chat-log">
        ${
          messages.length === 0
            ? '<div class="empty-state">Kick off the conversation and rally the Pi Duel Arena community.</div>'
            : messages
                .map((entry) => {
                  return `
...
function renderProfileView() {
  ...
  appEl.innerHTML = `
    <section class="card">
      ...
      <p class="subtitle">${escapeHtml(user.bio || 'Mock profile for the Pi Duel Arena world.')}</p>
    ...
    <section class="card">
      ...
      <div class="friend-list">
        ${
          friends.length === 0
            ? '<div class="empty-state">This player has not added any friends yet.</div>'
            : friends
